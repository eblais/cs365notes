...
And there is an infinite _tape_ split up into squares that each square contains exactly one symbol.
The machine has a _tape head_ that is always positioned over exactly one of the squares of the tape.
At each step in the execution, the Turing machine uses its internal state and the contents of the square of the tape that it sees to determine its next action.
The action itself consists of three parts: the new internal state it moves to, the symbol that is overwritten over the previous symbol on the square of tape, and a movement _Left_ or _Right_ to the square adjacent to the current one on the tape.
...
[Image of Transition diagram for TM]
...
>
> with $m,k \ge 1$ where
> * $$\Sigma = \{\mathbf{1}, \mathbf{2}, \ldots, \mathbf{m}\}$$ is the set of _internal states_,
> * $$\Gamma = \{\square,0,1,2,\ldots,k\}$$ is the _tape alphabet_, and
> * $$\delta \colon [m] \times \Gamma \to ([m] \cup \{\mathbf{A}, \mathbf{R}\}) \times \Gamma \times \{L, R\}$$ is the _transition function_.
{: .block-tip}

The state $$\mathbf{1}$$ is the _initial state_ of the Turing machine $$M$$.
...
> 
> * $$\mathbf{q} \in \Sigma \cup \{ \mathbf{A}, \mathbf{R} \}$$ represents the current state of the machine,
...
Every step of computation of a Turing machine changes its configuration.
We represent this change formally in the following way.

> **Definition.**
> For any strings $$w,y \in \Gamma^*$$, symbols $$a,b,c \in \Gamma$$, and states $$\mathbf{q} \in \Sigma$$ and $$\mathbf{r} \in \Sigma \cup \{ \mathbf{A}, \mathbf{R} \}$$, the configuration $$wa\mathbf{q}by$$ of the Turing machine $$M$$ _yields_ the configuration $$w \mathbf{r} acy$$, denoted
>
> $$
> wa \mathbf{q} by \implies w \mathbf{r} acy
> $$
> 
...
By simulating multiple steps of computation, a Turing machine can reach some (but not all!) other configurations.

> **Definition.**
> The configuration $$w \mathbf{q} y$$ _derives_ the configuration $$w' \mathbf{q'} y'$$ in the Turing machine $$M$$, denoted
> 
> $$
...
> More concretely, consider the machine $$M = (m,k,\delta)$$.
...
We say that a Turing machine $$S$$ _simulates_ machine $$M$$ on input $$x \in \{0,1\}^*$$ if $$S$$ accepts iff $$M$$ accepts $$x$$, $$S$$ rejects if and only if $$M$$ rejects $$x$$, and in both cases $$S$$ ends with the same content on the tape as $$M$$ does.

We can now show that there exists a single Turing machine that can simulate all the other ways in the following precise sense.
...
> A quick warning: it is not be immediately clear how all the steps below can be implemented using Turing machines. 
> For a great exercise, see if you can fill out the details for the implementation of some of these steps.
> 
> As a first step, the machine $$U$$ turns the input $$x$$ into the initial configuration of $$M$$ bgy adding a special symbol $$\mathbf{1}$$ at the beginning of $$x$$ to turn it into the string $$\mathbf{1}x$$.
...
As a very first application, we can use it to examine a strong claim: that Turing machines are as powerful as any other model of computation.
...
As you can verify directly, counter machines can compute many different languages.
...
The proof of the proposition shows the template for confirming the Church-Turing thesis on any other model of computation that you could define: to prove that Turing machines capture that model of computation, it suffices to show how to simulate the programs on these computers using a Turing machine.
...
_Eric Blais &copy;2023 &mdash; Last edited on Dec. 17, 2023_
